...
class TestSession(unittest.TestCase):
	...
	def test_cards_is_initially_empty(self):
		session = Session.from_deck_id(1)
		session.load_cards()
		self.assertEqual(len(session.cards), 0)

		session = Session.from_deck_id(2)
		session.load_cards()
		self.assertEqual(len(session.cards), 0)

		session = Session.from_deck_id(3)
		session.load_cards()
		self.assertEqual(len(session.cards), 0)
	...
	def test_load_cards_loads_most_recent_answer_history(self):
		...
		AnswerHistory(session.session_id, card_id, 1.12, True, two_seconds_ago).insert()
		session.load_cards()
		self.assertEqual(len(session.cards), 1)
		self.assertEqual(session.cards[0].answer_history.time_to_correct, 1.12)

		AnswerHistory(session.session_id, card_id, 2.12, True, DB.datetime_now()).insert()
		session.load_cards()
		self.assertEqual(len(session.cards), 1)
	...
	def test_from_deck_id_gives_new_session_if_stage_is_finished(self):
		...
		session2 = Session.from_deck_id(3)
