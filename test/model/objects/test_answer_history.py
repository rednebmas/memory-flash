...
class TestAnswerHistory(unittest.TestCase):
	...
	def test_first_review_from_last_day_reviewed_not_in_session(self):
		...
		AnswerHistory(session.session_id + 10, card_id, 1.12, True, two_seconds_ago).insert()
		AnswerHistory(session.session_id + 10, card_id, 2.12, True, DB.datetime_now()).insert()
