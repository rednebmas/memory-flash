...
def add_routes(app):
	...
	async def login(request):
		...
		try:
			...
			request.ctx.session['user_name'] = user.user_name
			request.ctx.session['user_id'] = user.user_id
	...
	async def logout(request):
		if 'user_name' in request.ctx.session:
			del request.ctx.session['user_name'] 
	...
	async def logout(request):
		user = User.from_db_id(request.ctx.session.get('user_id'))
